...
- This rollout remains CloudBase-only and Version 1-compatible: no new collection, field, or schema migration; no runtime MySQL migration; no dual-write; and no self-hosted HTTP API cutover. Deployment is still pending for `listUsers`, `listActivities`, `getAttendanceStats`, `listNotificationLogs`, `getActivityDetail`, `exportActivityRoster`, and Web Admin static hosting.
- Task 6 focused direct Jest passed with `10` suites and `138` tests. The mandated full direct Jest run is blocked by one pre-existing Web Admin login fixture: `tests/web-admin/app-login.test.js` supplies `{ items: [] }` for paginated user/activity mocks, but the current strict metadata contract rejects it. The run reports `84` passed and `1` failed suites, with `856` passed and `1` failed tests; this documentation-only task does not modify that out-of-scope fixture.
