...
- Task 7 CLI verification showed all six deployed functions as `Deployment completed` and `Active`; their remote modify times ranged from `15:56:37` through `15:58:57` (`listActivities` `15:57:04`, `getActivityDetail` `15:58:31`). Hosting uploaded `29/29` files to `/admin/`, and the hosted entry loaded `20260724-task-5-complete-statistics-export` for every local asset, with `pagination.js` before `app.js`.
- Authenticated admin smoke opened the existing session without showing QR/login. Activities showed `13` total and `13` rows on page `1/1`; users showed `102` total and navigated from `20` rows on page `1/6` to `20` rows on page `2/6` with previous enabled. Statistics showed shared total `22`, with attendance and cancellation independently reaching page `2/2`; the attendance tab's `17` then `2` visible rows are the documented projections of the shared paginated data. Notification logs showed `7` total, `7` successful rows on page `1/1`. `测试07221933` loaded `3` roster rows and `22` activity-log rows.
- CSV/XLSX roster menu actions completed without an application error, but Chrome download events/files could not be captured, so downloaded-file row counts were not inspected; unit tests cover parity. No activity or notification-log second page exists in current data, and ordinary-user denial was not live retested because the active session was admin. Console output had only unrelated extension warnings and no pagination-metadata or cloud-function application error.
