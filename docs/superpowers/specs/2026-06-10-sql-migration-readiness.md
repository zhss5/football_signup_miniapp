...
30. Keep Web Admin list responses API-shaped and additive: `listUsers`, Web Admin `listActivities`, `getAttendanceStats`, and `listNotificationLogs` return `{ items, total, limit, skip, hasMore }`. Existing callers that read `items` or `hasMore` remain compatible. The Web Admin uses a fixed `limit` of `20`; the backend owns filtering, final sorting, exact total calculation, and page slicing. `getAttendanceStats.statisticsType` is an additive stable enum (`attendance`, `cancellation`, or `both`). The first two values return one independently filtered page. `both` performs one shared source scan and returns `{ projections: { attendance, cancellation } }`, where each projection has independent `{ items, total, limit, skip, hasMore }` metadata. An omitted or unknown value preserves the legacy combined projection. The Web Admin applies a bounded request timeout so an unavailable backend cannot leave the loading state active indefinitely; this timeout is a client/API behavior and adds no stored field.
31. Keep CloudBase completeness internal to the backend. Every source collection that can exceed one CloudBase response page must apply supported business filters before reading, traverse with an `_id` cursor in deterministic order, deduplicate by `_id`, and apply a stable final sort with `_id` as the final tie-breaker before returning an offset page or aggregate. `limit` and `skip` remain the Version 2 public contract and map directly to SQL `LIMIT/OFFSET`; they are not a permanent commitment to offset pagination, and a future cursor must be additive while Version 2 clients remain active.
32. Keep a single activity roster unpaginated at the public row level. `getActivityDetail` and `exportActivityRoster` must fully read `activityId` plus `status: 'joined'` registrations before rendering or exporting, restrict user-profile reads to real roster OpenIDs, and use one exact flattened order: numeric team `sort`, team `_id` as the tie-breaker, `joinedAt`, participant display name resolved as `signupName`, then `displayName`, then `preferredName`, then `userOpenId`, then registration `_id` as the malformed historical-row fallback, and registration `_id` as the final sort tie-breaker. Flattened `getActivityDetail` members must exactly match `exportActivityRoster` rows. If a joined registration has an empty or dangling `teamId`, both APIs materialize the same API-only `未分队` fallback group after real teams; a dangling non-empty ID remains stable, while an empty ID receives a collision-free reserved ID. The fallback group is not persisted to CloudBase or SQL. SQL migration validation must report and repair these orphan registrations before foreign-key enforcement. Ordinary users keep existing roster-detail visibility; organizer/admin registration-management and export boundaries remain server-enforced.
33. Treat Web Admin pagination metadata as a backend contract, not display hints. The client must require non-negative integer `total` and `skip`, positive integer `limit`, and boolean `hasMore`; on invalid or inconsistent metadata it preserves the previously rendered rows and restores controls. Statistics CSV/XLSX export must re-read every validated page for the active filters, reject inconsistent totals, item counts, `hasMore`, or progress, and abort rather than download a partial file.
...
- SQL-backed `listUsers`, Web Admin `listActivities`, `getAttendanceStats`, and `listNotificationLogs` return the same exact `{ items, total, limit, skip, hasMore }` metadata as CloudBase for the same authenticated caller and filters. Totals are filtered totals, `hasMore` is consistent with the returned page, and SQL checks include empty, first, middle, and terminal pages at the fixed Web Admin limit of `20`. Attendance and cancellation statistics are validated independently with `statisticsType`; cancellation-only participants never reduce an attendance page below its expected size, and each projection has its own exact total and page sequence. The `statisticsType = both` response must match two independent projection queries while using one backend source scan.
- SQL-backed collection and aggregate replacements return the same complete candidate set as CloudBase cursor reads before final filtering, sorting, and pagination. Validate deterministic `_id` tie-breakers where primary sort values collide.
- SQL-backed activity detail and roster export return every joined registration for one activity and exactly preserve the shared flattened order: numeric team `sort`, team `_id`, `joinedAt`, participant display-name fallback (`signupName`, `displayName`, `preferredName`, `userOpenId`), then registration `_id`. Flattened activity-detail members and roster-export rows must match, and proxy rows must not resolve through real-user profile lookups.
- SQL-backed attendance and cancellation CSV/XLSX export reads traverse every validated API page for the same filters and abort on an inconsistent total, page size, `hasMore`, or cursor/offset progress instead of producing a partial file.
