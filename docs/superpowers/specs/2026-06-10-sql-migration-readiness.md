...
32. Keep a single activity roster unpaginated at the public row level. `getActivityDetail` and `exportActivityRoster` must fully read `activityId` plus `status: 'joined'` registrations before rendering or exporting, restrict user-profile reads to real roster OpenIDs, and retain team, join-time, participant-name, then registration-ID ordering. Ordinary users keep existing roster-detail visibility; organizer/admin registration-management and export boundaries remain server-enforced.
...
- SQL-backed activity detail and roster export return every joined registration for one activity, use the same team/join-time/participant/registration-ID ordering, and do not resolve proxy rows through real-user profile lookups.
